...
bool deinitSPISDCard(bool log){
  if (numSdCallers >= 0)
    numSdCallers--;
  if (log) ESP_LOGV(__FUNCTION__, "SD callers %d", numSdCallers);
  if (numSdCallers == 0) {
    ...
    {
      getAppState()->sdCard = item_state_t::ACTIVE;
      if (log) ESP_LOGD(__FUNCTION__, "Unmounted SD Card");
    } else {
      getAppState()->sdCard = item_state_t::ERROR;
      if (log) ESP_LOGE(__FUNCTION__, "Failed to unmount SD Card");
      return false;
    }
    ...
    if (log) ESP_LOGV(__FUNCTION__,"Postponing SD card umount");
    return ESP_OK;
  }
}
  ...
  if (numSdCallers <= 0) {
    ...
    if ((ret=spi_bus_initialize((spi_host_device_t)host.slot, &bus_cfg, 1)) == ESP_OK) {
      if ((ret=esp_vfs_fat_sdspi_mount(mount_point, &host, &slot_config, &mount_config, &card)) != ESP_OK)
      {
        ...
        if (ret == ESP_FAIL)
        {
          if (log) ESP_LOGE(__FUNCTION__, "Failed to mount filesystem. "
                                "If you want the card to be formatted, set format_if_mount_failed = true.");
        }
        else
        {
          if (log) ESP_LOGE(__FUNCTION__, "Failed to initialize the card (%s). "
                                "Make sure SD card lines have pull-up resistors in place.",
                  esp_err_to_name(ret));
          //ESP_LOGE(__FUNCTION__,"slot_config.gpio_miso:%d slot_config.gpio_mosi:%d slot_config.gpio_sck:%d,slot_config.gpio_sck:%d",slot_config.gpio_miso,slot_config.gpio_mosi,slot_config.gpio_sck,slot_config.gpio_cs);
        }
      ...
      if (log) ESP_LOGD(__FUNCTION__, "SD card mounted %d", (int)card);
      if (LOG_LOCAL_LEVEL >= ESP_LOG_DEBUG)
        sdmmc_card_print_info(stdout, card);
      if (log) ESP_LOGD(__FUNCTION__, "SD card mounted %d", (int)card);
      ...
      //f_mkdir("/logs");
      //f_mkdir("/sent");
      //const esp_vfs_fat_mount_config_t Fconf = {
      //        .format_if_mount_failed = true,
      //        .max_files = 6,
      //        .allocation_unit_size = CONFIG_WL_SECTOR_SIZE
      //};
      //esp_err_t err = esp_vfs_fat_spiflash_mount("/lfs", "storage", &Fconf, &s_wl_handle);
      //if (err != ESP_OK) {
      //    ESP_LOGE(__FUNCTION__, "Failed to mount FATFS (%s)", esp_err_to_name(err));
      //    return false;
      //}
      ...
      if (log) ESP_LOGW(__FUNCTION__,"Error initing SPI bus %s",esp_err_to_name(ret));
    } else {
      if (log) ESP_LOGE(__FUNCTION__,"Error initing SPI bus %s",esp_err_to_name(ret));
      return false;
    }
    numSdCallers=1;
  } else {
...
FILE * fopen (const char * _name, const char * _type,bool createDir, bool log){
  if (!_name || (strlen(_name)==0)) {
    if (log) ESP_LOGE(__FUNCTION__,"No file name given");
    return NULL;
  }
  char buf[255];

  if (log) ESP_LOGV(__FUNCTION__,"Opening %s's folder with perm %s create folder %d",_name,_type,createDir);
  if (createDir) {
    if (log) ESP_LOGV(__FUNCTION__,"Validating %s's folder",_name);
    DIR* theFolder;
    ...
    if (closingMark>folderName){
      *closingMark=0;
      if (log) ESP_LOGV(__FUNCTION__,"Folder is %s",folderName);
      uint32_t flen=strlen(folderName);
      if ((theFolder = opendir(folderName)) == NULL) {
        if (log) ESP_LOGV(__FUNCTION__,"Folder %s does not exist",folderName);
        closingMark=strchr(folderName+1,'/');
        *closingMark=0;
        while (strlen(folderName) <= flen){
          if (log) ESP_LOGV(__FUNCTION__,"Checking Folder %s",folderName);
        ...
        if (log) ESP_LOGV(__FUNCTION__,"Folder %s does exist",folderName);
        closedir(theFolder);
      }
