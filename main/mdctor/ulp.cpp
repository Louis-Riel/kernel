    ...
    if (level==0){
        ...
        switch (sstate) {
            ...
            case serial_state_t::start_bit:
                if (periodLen<bit_tick_len){
                    ...
                    //printf(".");
                    //ESP_LOGI(__PRETTY_FUNCTION__,"Start periodLen:%d bit_tick_len:%dstart:%" PRId64 " end:%" PRId64 ".",periodLen,bit_tick_len,curCalib.rizingTs,curCalib.loweringTs);
                } else {
                    sstate=serial_state_t::stop_bit;
                    //printf("&");
                    ESP_LOGW(__PRETTY_FUNCTION__,"Bad start periodLen:%d bit_tick_len:%dstart:%" PRId64 " end:%" PRId64 ".",periodLen,bit_tick_len,curCalib.rizingTs,curCalib.loweringTs);
                }
                curByte=0;
                curBytePos=1;
                break;
        ...
        switch (sstate) {
            ...
            case serial_state_t::stop_bit:
                if (curBytePos==9) {
                    ESP_LOGI(__PRETTY_FUNCTION__,"WE GOT A CHAR:%d",curByte);
                }
...
void initUlp(gpio_num_t pinNo) {
    ...
    ESP_LOGI(__PRETTY_FUNCTION__,"portTICK_PERIOD_MMS:%f rtc_8md256_period:%d rtc_fast_freq_hz:%d bit tick len:%d",portTICK_PERIOD_MMS,rtc_8md256_period,rtc_fast_freq_hz,bit_tick_len);
