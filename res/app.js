...
class IntInput extends React.Component {
    ...
    render() {
        return e("label", { key: genUUID(), className: "editable", id: `lbl${this.id}`, key: this.id },
            ...
            e("input", { key: genUUID(), type: "number", value: this.state.value, onChange: this.toggleChange, id: `in${this.id}` }));
    }
}
...
class ControlPannel extends React.Component {
    ...
    render() {
        return e('fieldset', { key: genUUID(), id: "controls", key: this.id }, [
            ...
            e(BoolInput, { key: genUUID(), label: "Auto Refresh", onOn: setupWs, onOff: setupWs, initialState: () => { return ws != null }, id: `at${this.id}` })
        ]);
    }
}
...
class MainAppState extends React.Component {
    ...
    refreshState() {
        return new Promise((resolve, reject) => {
            ...
            var timer1 = setTimeout(() => controller.abort(), 3000);
            fetch(`${httpPrefix}/status/`, {
                    method: 'post',
                ...
                .then(jjson => {
                    clearTimeout(timer1);
                    controller = new AbortController();
                    timer1 = setTimeout(() => controller.abort(), 3000);
                    fetch(`${httpPrefix}/status/app`, {
                        ...
                        .then(ajson => {
                            ...
                            fetch(`${httpPrefix}/status/tasks`, {
                                ...
                                .then(tjson => {
                                    clearTimeout(timer1);
                                    Object.assign(jjson, ajson)["tasks"] = Object.values(tjson);
                                    this.setState({ lastFetched: new Date(), jsons: jjson });
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if (this.state.lastFetched < this.state.stateReqDt) {
            this.refreshState();
        }
    }
...
class StorageViewer extends React.Component {
    ...
    getFileLink(file) {
        if (file.ftype == "folder") {
            ...
            return e("a", { href: `${this.state.path}/${file.name}` }, file.name);
        }
    }
        ...
        fetch(`${httpPrefix}/files` + this.state.path, {
            ...
            data.json()
                ...
                .then(files => {
                    ...
                    var fileStatsToFetch = files.filter(file => file.ftype == "file" && !file.refreshed);
...
function refreshStatus(clicked) {
    ...
    if (stateManager == null) {
        stateManager = ReactDOM.render(e(MainAppState, { key: genUUID(), stateReqDt: new Date() }), domContainer);
    } else {
